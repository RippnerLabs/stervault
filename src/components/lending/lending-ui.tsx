...
export function LendingCreate() {
  const { initialize } = useLendingProgram()
      ...
      {accounts.isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {accounts.data?.map((account) => (
            <LendingCard key={account.publicKey.toString()} account={account.publicKey} />
          ))}
        </div>
      ) : (
...
function LendingCard({ account }: { account: PublicKey }) {
